...
Different layers may have different configurations. The subsequent sections
...
These layers are categorized according to their functionalities,
  ...
  * Parser layers for parsing features, labels, etc. from records, into [Blob].
...
Data layers load training/testing data and convert them into [Record]()s.
The data source can be disk file, HDFS, database or network.

##### ShardDataLayer

[ShardDataLayer] is used to read data from disk file. The file should be created
using [DataShard] class. With the data file prepared, users configure the layer as
...
[LabelLayer] is used to parse a single label from each Record. Consequently, it
...
##### RGBImageLayer
[RGBImageLayer] parses the RGB values of one image from each Record. It may also
...
[PrefetchLayer] embeds data layers and parser layers to do data prefeching.
...
[InnerProductLayer] is fully connected with its (single) source layer.
...
[PoolingLayer] is used to do a normalization (or averaging or sampling) of the
...
[TanhLayer] uses the tanh as activation function, i.e., `f(x)=tanh(x)`.
...
##### Dropout Layer
[DropoutLayer] is a layer that randomly dropouts some inputs.
...
[SoftmaxLossLayer] is a combination of the Softmax transformation and
...
[ConcateLayer] connects more than one source layers to concatenate their feature
...
[SliceLayer] connects to more than one destination layers to slice its feature
blob along given dimension.
...
[SplitLayer] connects to more than one destination layers to replicate its
feature blob.
...
[BridgeSrcLayer] & [BridgeDstLayer] are utility layers assisting data (e.g., feature or
...
in the `data_` [Blob] (A Blob is a chunk of memory space, proposed in
...
The newly defined layer should be registered in users' [main.cc]({{ BASE_PATH }}/docs/programming-guide) by
