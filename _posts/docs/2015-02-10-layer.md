...
The configurations of three layers from the [MLP example]({{ BASE_PATH }}/docs/mlp) is shown below,
...
These layers are categorized according to their functionalities,
  ...
  * Parser layers for parsing features, labels, etc. from records, into [Blob]({{ BASE_PATH }}/api/classsinga_1_1Blob.html).
  * Neuron layers for feature transformation, e.g., [convolution]({{ BASE_PATH }}/api/classsinga_1_1ConvolutionLayer.html), [pooling]({{ BASE_PATH }}/api/classsinga_1_1PoolingLayer.html), dropout, etc.
...
[ShardDataLayer]({{ BASE_PATH }}/api/classsinga_1_1ShardDataLayer.html) is used to read data from disk file. The file should be created
using [DataShard]({{ BASE_PATH }}/api/classsinga_1_1DataShard.html) class. With the data file prepared, users configure the layer as
...
[LabelLayer]({{ BASE_PATH }}/api/classsinga_1_1LabelLayer.html) is used to parse a single label from each Record. Consequently, it
...
##### RGBImageLayer
[RGBImageLayer]({{ BASE_PATH }}/api/classsinga_1_1RGBImageLayer.html) parses the RGB values of one image from each Record. It may also
...
[PrefetchLayer]({{ BASE_PATH }}/api/classsinga_1_1PrefetchLayer.html) embeds data layers and parser layers to do data prefeching.
...
[ConvolutionLayer]({{ BASE_PATH }}/api/classsinga_1_1ConvolutionLayer.html) conducts convolution transformation.
...
[InnerProductLayer]({{ BASE_PATH }}/api/classsinga_1_1InnerProductLayer.html) is fully connected with its (single) source layer.
...
[PoolingLayer]({{ BASE_PATH }}/api/classsinga_1_1PoolingLayer.html) is used to do a normalization (or averaging or sampling) of the
...
[ReLuLayer]({{ BASE_PATH }}/api/classsinga_1_1ReLULayer.html) has rectified linear neurons, which conducts the following
transformation, `f(x) = Max(0, x)`. It has no specific configuration fields.

##### TanhLayer

[TanhLayer]({{ BASE_PATH }}/api/classsinga_1_1TanhLayer.html) uses the tanh as activation function, i.e., `f(x)=tanh(x)`.
...
##### Dropout Layer
[DropoutLayer]({{ BASE_PATH }}/api/asssinga_1_1DropoutLayer.html) is a layer that randomly dropouts some inputs.
...
##### LRNLayer
[LRNLayer]({{ BASE_PATH }}/api/classsinga_1_1LRNLayer.html), (Local Response Normalization), normalizes over the channels.
...
[SoftmaxLossLayer]({{ BASE_PATH }}/api/classsinga_1_1SoftmaxLossLayer.html) is a combination of the Softmax transformation and
...
[ConcateLayer]({{ BASE_PATH }}/api/classsinga_1_1ConcateLayer.html) connects more than one source layers to concatenate their feature
...
[SliceLayer]({{ BASE_PATH }}/api/classsinga_1_1SliceLayer.html) connects to more than one destination layers to slice its feature
blob along given dimension.
...
[SplitLayer]({{ BASE_PATH }}/api/classsinga_1_1SplitLayer.html) connects to more than one destination layers to replicate its
feature blob.
...
[BridgeSrcLayer]({{ BASE_PATH }}/api/classsinga_1_1BridgeSrcLayer.html) & [BridgeDstLayer]({{ BASE_PATH }}/api/classsinga_1_1BridgeDstLayer.html) are utility layers assisting data (e.g., feature or
...
in the `data_` [Blob]({{ BASE_PATH }}/api/classsinga_1_1Blob.html) (A Blob is a chunk of memory space, proposed in
...
objective loss w.r.t. the `data_` Blob. It is necessary in [BP algorithm]({{ BASE_PATH }}/api/classsinga_1_1BPWorker.html),
hence we put it as a member of the base class. For [CD algorithm]({{ BASE_PATH }}/api/classsinga_1_1CDWorker.html), the `grad_`
...
[Neural net partitioning]({{ BASE_PATH }}/docs/neural-net/#neural-net-partitioning). Typically, the
...
trained using [BP algorithm]({{ BASE_PATH }}/docs/train-one-batch/#back-propagation),
...
[CD algorithm]({{ BASE_PATH }}/docs/train-one-batch/#contrastive-divergence), which
